...
/**
 * Followup aggregation for the double min.
 */
object FollowUpDoubleMin extends SimpleFollowUpDerivator
{
   override def matches(au: AggUpdate): Boolean = au.`type` == DoubleSupport.`type` && au.aggregation == "min"
   ...
   override def aggregate(au: AggUpdate, ta: TimeAggregator, history: List[(LocalDateTime, JsValue)]): List[AggUpdate] = {
      ...
      List(
         AggUpdate(au.ts, ta.name, ta.shunk(au.datetime), DoubleSupport.`type`, DoubleSupport.asJson(min), "min")
      )
   }
}
